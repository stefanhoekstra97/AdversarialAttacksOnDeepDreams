...
class Cifar_nn(nn.Module):
    def __init__(self, input_shape: tuple = (128, 3, 32, 32), device='auto'):
        super().__init__()
        if device == 'auto':
        ...
        self.forward(torch.ones(input_shape, device=device))
    ...
    def softmax_switch(self, return_softmax:bool = False):
